...
### 📝 方法1: admin.html を使う（推奨・初心者向け）

**最も簡単な方法です！** Webブラウザで説明文を編集できます。

#### 手順:

1. **admin.htmlを開く**
   - エクスプローラーで作業フォルダを開く
   - `admin.html` ファイルをダブルクリック
   - ブラウザで開きます（Chrome、Edge、Firefox など）

2. **説明文を編集**
   - 画面に表示される7つのテキストボックスで説明文を編集
   - HTMLタグを使って文字装飾も可能（太字、色など）
...
4. **コードを生成してコピー**
   - 「📋 Pythonコードを生成」ボタンをクリック
   - 「📋 クリップボードにコピー」ボタンをクリック

5. **Pythonファイルに貼り付け**
   - `excel_to_html_converter.py` をメモ帳で開く
   - 35行目付近の `INFO_TEXT_LINES = [...]` の部分を探す
   - その部分を削除して、コピーしたコードを貼り付け
   - ファイルを保存
...
- **下線**: `<u>テキスト</u>`
- **赤色**: `<span style="color: red;">テキスト</span>`
- **青色**: `<span style="color: #0066cc;">テキスト</span>`
- **リンク**: `<a href="URL">テキスト</a>`

**例:**
```
○ <strong>{date}</strong> 次世代自動車振興センター
```

---

### 📄 方法2: Pythonファイルを直接編集（上級者向け）

Pythonファイルを直接編集することもできます。

#### 概要

生成されるHTMLの説明文（日付や注意書きなど）は、Pythonスクリプト内の設定で簡単に変更できます。

#### 説明文の変更方法

1. **Pythonスクリプトを開く**
   - `excel_to_html_converter.py` をテキストエディタで開く
   - メモ帳でも可能ですが、できればVisual Studio CodeやNotepad++などのコードエディタを推奨

2. **設定セクションを探す**
   - ファイルの先頭付近（35行目あたり）に以下のセクションがあります：
   ```python
   # ============================================================================
...
3. **テキストを編集**
   - `INFO_TEXT_LINES` のリスト内のテキストを編集します
   - 例：
   ```python
   INFO_TEXT_LINES = [
       "○ {date} 次世代自動車振興センター",
...
```python
INFO_TEXT_LINES = [
    "○ {date} <strong>次世代自動車振興センター</strong>",
    "○ <em>{year}年度は {date} までの集計です</em>",
    "※<span style="color: #d32f2f;"><strong>重要:</strong></span> {year}年度の補助金交付台数等については、<u>現在審査中</u>のものもあるため...",
]
```

### プレースホルダー（自動挿入される値）

説明文内で以下のプレースホルダーが使用できます：

- `{date}` → 現在の日付（例: 2025年11月22日）
- `{year}` → 現在の年（例: 2025）

これらは実行時に自動的に置き換えられます。
...
**変更後（太字と色を追加）:**
```python
"○ {date} <strong style=\"color: #0066cc;\">次世代自動車振興センター</strong>",
```

**注意:** ダブルクォート（`"`）を文字列内で使う場合は、バックスラッシュ（`\`）でエスケープしてください： `\"`
...
- **行を削除する**: 不要な行をコメントアウト（`#`を先頭に追加）または削除
  ```python
  INFO_TEXT_LINES = [
      "○ {date} 次世代自動車振興センター",
      # "○ この行は表示されません",  # ← コメントアウト
...
**ヒント:** 
- Windowsの場合: エクスプローラーでファイル名をShiftキーを押しながら右クリック→「パスのコピー」で正確なパスを取得できます
- Macの場合: Finderでファイルを選択し、Optionキーを押しながら右クリック→「"ファイル名"のパス名をコピー」で正確なパスを取得できます
